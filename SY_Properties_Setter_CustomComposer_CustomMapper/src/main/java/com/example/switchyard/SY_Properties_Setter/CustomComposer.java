...
public class CustomComposer extends HttpMessageComposer {
    ...
    public HttpBindingData decompose(Exchange exchange, HttpBindingData target) throws Exception {
        ...
        if (null != exchange.getContext().getPropertyValue("errorProperty")) {
            String errorProperty = exchange.getContext().getPropertyValue("errorProperty");
            System.out.println("errorProperty is:" + errorProperty);
            exchange.getMessage().setContent(errorProperty);        	
        }
